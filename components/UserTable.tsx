...
import { AlertTriangle, CheckCircle, Clock, Search, Download, ChevronLeft, ChevronRight, Filter, Users, PlayCircle, Timer, X, ArrowLeft, Bug, Copy, ShieldAlert } from 'lucide-react';
import { isDateInRange } from '../services/dataProcessing';
...
type RiskFilterType = 'All' | 'Deep Account' | 'Bot Activity' | 'Sybil' | 'Email Risk';

export const UserTable: React.FC<UserTableProps> = ({ data, initialFilters, onBack, startDate, endDate }) => {
  ...
  // Handle Global Date Params parsing
  ...
  const filteredData = useMemo(() => {
    ...
    if (dateRange.start && dateRange.end) {
        // We filter by Registration Date (createdAt) as the primary timestamp for "Developers"
        result = result.filter(d => isDateInRange(d.createdAt, dateRange.start, dateRange.end));
    }
    ...
    // 4. Granular Risk Filter
    if (riskFilter !== 'All') {
        if (riskFilter === 'Deep Account') {
            result = result.filter(d => d.computed_riskFlags.some(f => f.includes('Batch Pattern')));
        } else if (riskFilter === 'Bot Activity') {
            result = result.filter(d => d.computed_riskFlags.some(f => f.includes('Bot') || f.includes('Speed')));
        } else if (riskFilter === 'Sybil') {
            result = result.filter(d => d.computed_riskFlags.some(f => f.includes('Sybil')));
        } else if (riskFilter === 'Email Risk') {
            result = result.filter(d => d.computed_riskFlags.some(f => f.includes('Email')));
        }
    }
    ...
    return result;
  }, [data, searchQuery, statusFilter, communityFilter, riskFilter, dateRange]);
  ...
  useEffect(() => setCurrentPage(1), [searchQuery, statusFilter, communityFilter, riskFilter]);

  const clearFilters = () => { setSearchQuery(''); setStatusFilter('All'); setCommunityFilter('All'); setRiskFilter('All'); };
  ...
  const getRiskIcon = (reason: string) => {
      ...
      if (r.includes('bot')) return <Timer className="w-3 h-3" />;
      if (r.includes('sybil')) return <Users className="w-3 h-3" />;
  ...
  return (
    <div className="space-y-6 fade-in-up">
          ...
          <div className="flex flex-col md:flex-row gap-3 w-full xl:w-auto">
              ...
              <div className="relative flex-1 md:w-48">
                  ...
                  >
                      ...
                      <option value="Flagged">Any Risk</option>
              ...
              <div className="relative flex-1 md:w-48">
                  ...
                  <select 
                      value={riskFilter} 
                      onChange={(e) => setRiskFilter(e.target.value as RiskFilterType)} 
                      className="w-full pl-10 pr-8 py-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium text-slate-900 dark:text-white focus:ring-2 focus:ring-red-500 outline-none appearance-none cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 shadow-sm"
                  >
                      <option value="All">All Risks</option>
                      <option value="Deep Account">Deep Accounts (Pattern)</option>
                      <option value="Bot Activity">Bots / Speed Runs</option>
                      <option value="Sybil">Sybil (Wallets)</option>
                      <option value="Email Risk">Disposable Emails</option>
                  </select>
              </div>

              <div className="flex items-center gap-2">
                  {(statusFilter !== 'All' || communityFilter !== 'All' || riskFilter !== 'All' || searchQuery) && (
      ...
      {startDate && endDate && (
          <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 dark:bg-[#2a00ff]/10 text-blue-700 dark:text-blue-300 rounded-lg text-xs font-bold border border-blue-100 dark:border-blue-900/30 w-fit">
