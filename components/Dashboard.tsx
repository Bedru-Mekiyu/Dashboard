...
interface DashboardProps {
  ...
  // AI Props from Parent (Persistence)
  aiSummary: string;
  setAiSummary: (s: string) => void;
  aiContext: string;
  setAiContext: (s: string) => void;
}
...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const communities = useMemo(() => {
    const uniqueCodes = new Set(data.map(d => d.partnerCode).filter(c => c && c !== 'UNKNOWN'));
    const sortedCodes = Array.from(uniqueCodes).sort();
    
    // AUTO-DASHBOARD GENERATION LOGIC:
    // If there is only one partner code in the dataset, it means the user is restricted 
    // OR we are filtered down to a single entity.
    // In this case, removing "All" forces the dashboard to appear as a "Dedicated Partner Dashboard".
    if (sortedCodes.length === 1) {
        return [[sortedCodes[0], sortedCodes[0]]];
    }

    return [['All', 'Global (All)'], ...sortedCodes.map(code => [code, code])];
  }, [data]);

  // Auto-select if 'All' is invalid or not in list (Scope Enforcement)
  useEffect(() => {
      const availableCodes = communities.map(c => c[0]);
      
      // FIX: Scope Enforcement for Community Admins
      // If the user is restricted to a single community, the dropdown will only have 1 option.
      // We must force the state (both active and pending) to match this option immediately.
      // This prevents the "blank dropdown" issue when the select input is disabled.
      if (communities.length === 1) {
          const enforcedCode = communities[0][0];
          
          if (activeCommunity !== enforcedCode) {
              setActiveCommunity(enforcedCode);
          }
          if (pendingCommunity !== enforcedCode) {
              setPendingCommunity(enforcedCode);
          }
          return;
      }

      // Standard Validation: If current selection is invalid, reset to default
      if (!availableCodes.includes(activeCommunity)) {
          // Prefer 'All' if available, otherwise first option
          const defaultSelection = availableCodes.includes('All') ? 'All' : availableCodes[0];
          setActiveCommunity(defaultSelection);
          setPendingCommunity(defaultSelection);
      }
  }, [communities, activeCommunity, pendingCommunity]);
  ...
  const leaderboardData = useMemo(() => {
      return generateLeaderboard(communityFilteredData, calculatedDateRange.start, calculatedDateRange.end);
  }, [communityFilteredData, calculatedDateRange]);

  // --- AI PERSISTENCE LOGIC ---
  
  // Create a context string to detect changes in data or filters
  const currentContext = useMemo(() => {
      const dataFingerprint = data.length > 0 ? data.length + data[0].id : 'empty';
      return `${activeCommunity}-${timeframe}-${startDate}-${endDate}-${dataFingerprint}`;
  }, [activeCommunity, timeframe, startDate, endDate, data]);

  // Defined before useEffect to be accessible inside it
  const handleGenerateSummary = async () => {
      ...
      try {
        if (!process.env.API_KEY) {
            ...
            // Mark context as synced even if error, to prevent loop
            setAiContext(currentContext); 
        } else {
            const summary = await generateExecutiveSummary(metrics, fullContext);
            setAiSummary(summary);
            setAiContext(currentContext); // Sync context on success
        }
      } catch (e) { 
  ...
  // 1. Detect Stale Context
  useEffect(() => {
      if (data.length === 0) {
           if (aiSummary !== "No data available. Please upload a CSV file.") {
              setAiSummary("No data available. Please upload a CSV file.");
              // Context is irrelevant if no data
           }
           return;
      }
      
      // If the context stored in App.tsx doesn't match current dashboard state, 
      // it means we have new data/filters and need to reset/regenerate.
      if (currentContext !== aiContext) {
           // Only reset if we actually have data to analyze. 
           // If we are just initializing (or returning to default), and aiSummary is "Waiting...", that's fine.
           // If aiSummary has old text, we reset it to trigger the next effect.
           if (aiSummary !== "Waiting for data...") {
               setAiSummary("Waiting for data...");
           }
      }
  }, [currentContext, aiContext, data.length, aiSummary, setAiSummary]);

  // 2. Trigger Generation (Existing logic adapted)
  useEffect(() => {
      if (metrics && !loadingAi) {
          // Automatically generate summary if we are in a 'waiting' state AND we have valid metrics
          if ((aiSummary === "Waiting for data..." || aiSummary === "No data available. Please upload a CSV file.") && metrics.totalRegistered > 0) {
              handleGenerateSummary();
          }
      }
  }, [metrics, aiSummary, loadingAi]); // Removed data.length dependency as it is handled by context check
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="glass-panel p-6 rounded-2xl bg-white dark:bg-[#1c1b22]">
        ...
        {/* Changed items-end xl:items-center to items-end to ensure baseline alignment */}
