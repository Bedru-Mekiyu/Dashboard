...
export const isDateInRange = (dateStr: string | null, startDate: Date | null, endDate: Date | null): boolean => {
...
export const generateLeaderboard = (data: DeveloperRecord[], startDate: Date | null, endDate: Date | null) => {
    // Group by Code (Unique ID)
    const counts: Record<string, { count: number, name: string }> = {};
    
    data.forEach(r => {
        if (!r.partnerCode || r.partnerCode === 'UNKNOWN') return;
        if (r.finalGrade === 'Pass') {
            // Apply Date Filter if filtering is active
            if (!isDateInRange(r.completedAt, startDate, endDate)) {
                return;
            }

            if (!counts[r.partnerCode]) {
                // Strictly use partnerCode for display as per user request
                counts[r.partnerCode] = { count: 0, name: r.partnerCode };
            }
